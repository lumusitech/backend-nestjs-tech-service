...
import { FilterSupplierDto } from './filter-supplier.dto';

async function transformAndValidate(
  params: Record<string, unknown>,
): Promise<FilterSupplierDto> {
  const dto = plainToInstance(FilterSupplierDto, params);
  await validate(dto);
  return dto;
}

describe('FilterSupplierDto', () => {
  ...
  describe('date range validation', () => {
    ...
    it('should accept dateTo without dateFrom', async () => {
      const dto = plainToInstance(FilterSupplierDto, { dateTo: '2026-12-31' });
      const errors = await validate(dto);
      expect(errors).toHaveLength(0);
    });
  });
